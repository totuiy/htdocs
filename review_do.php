...
// fetch data of rate
$recordSet = mysql_query("SELECT * FROM drivers WHERE id = $id");
if (!$recordSet) { die('Failed of query: '.mysql_error()); }
$data = mysql_fetch_assoc($recordSet);
/*echo '<br><br><br><br>';
echo 'ID = '.$data["id"];
echo '<br>Rate(old) = '.$data["rate"];
echo '<br>Rate(new) = '.$rate;
 */

// calculate new rate
$new_rate = $data["rate"] + $rate;
// echo '<br>Rate(sum) = '.$new_rate;
$new_rate = $new_rate / 2;
// echo "<br>new_rate = $new_rate";

// update rate
mysql_query("UPDATE drivers SET rate=$new_rate WHERE id=$id"); 
...
<html lang="en">
  <head>
    ...
    <title>Thank you for your review! TEST</title>
